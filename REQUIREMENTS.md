...
| I-1.5 | The remaining toolchain questions are answered before Terraform is written | **Not met — three questions open in [`docs/floci.md`](docs/floci.md), down from four; the EKS module one is answered. This is the gate on I-2.x.** The mechanism now exists: `spike/floci-verification/` and `spike-floci.yml` apply against floci in CI and report the answers. Met once that run's answers are moved into `floci.md` and the spike is deleted |
